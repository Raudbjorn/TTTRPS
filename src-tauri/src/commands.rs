...
pub async fn check_copilot_auth(
    ...
    let (copilot_token_expires_at, has_github_token) = if authenticated {
        match state.copilot_gate.get_token_info().await? {
            Some(token_info) => (
                token_info.copilot_expires_at,
                token_info.has_github_token(),
            ),
            None => (None, false),
        }
    } else {
...
pub async fn get_copilot_usage(
    state: State<'_, AppState>,
) -> Result<CopilotUsageInfo, String> {
    ...
    Ok(CopilotUsageInfo {
        ...
        completions: usage.quota_snapshots.completions.map(CopilotQuotaDetail::from),
        premium_requests: usage.quota_snapshots.premium_requests.map(CopilotQuotaDetail::from),
        code_completions: usage.quota_snapshots.code_completions.map(CopilotQuotaDetail::from),
    })
}
        ...
        let (supports_chat, supports_tools, supports_vision, max_context, max_output) =
            if let Some(caps) = &model.capabilities {
                ...
                (
                    ...
                    caps.limits.as_ref().map(|l| l.max_context_window_tokens as u64),
