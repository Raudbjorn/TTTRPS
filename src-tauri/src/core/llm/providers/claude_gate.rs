//! Claude Gate Provider Implementation
//!
...
//!
//! ```rust,no_run
//! use crate::core::llm::providers::ClaudeGateProvider;
//!
//! // Using file storage (default)
//! let provider = ClaudeGateProvider::new().unwrap();
//!
//! // Using keyring storage
//! let provider = ClaudeGateProvider::with_keyring().unwrap();
//!
...
use crate::claude_gate::models::ContentDelta;
#[cfg(feature = "keyring")]
...
/// Status of the Claude Gate provider
...
impl Default for ClaudeGateStatus {
    fn default() -> Self {
        Self {
            authenticated: false,
...
/// Claude Gate provider using OAuth authentication.
///
...
pub struct ClaudeGateProvider {
    /// The underlying Claude client (type-erased for flexibility)
    client: Arc<dyn ClaudeGateClient>,
    /// Model to use
    model: String,
...
#[async_trait]
trait ClaudeGateClient: Send + Sync {
    async fn is_authenticated(&self) -> crate::claude_gate::Result<bool>;
    async fn start_oauth_flow(&self) -> crate::claude_gate::Result<String>;
    async fn complete_oauth_flow(
        &self,
        code: &str,
        state: Option<&str>,
    ) -> crate::claude_gate::Result<crate::claude_gate::TokenInfo>;
    async fn logout(&self) -> crate::claude_gate::Result<()>;
    async fn get_token_info(&self) -> crate::claude_gate::Result<Option<crate::claude_gate::TokenInfo>>;
    async fn send_message(
        &self,
        model: &str,
        max_tokens: u32,
        messages: Vec<crate::claude_gate::Message>,
        system: Option<String>,
        temperature: Option<f32>,
    ) -> crate::claude_gate::Result<MessagesResponse>;
...
#[async_trait]
impl ClaudeGateClient for FileStorageClient {
    async fn is_authenticated(&self) -> crate::claude_gate::Result<bool> {
...
#[cfg(feature = "keyring")]
#[async_trait]
impl ClaudeGateClient for KeyringStorageClient {
    async fn is_authenticated(&self) -> crate::claude_gate::Result<bool> {
        self.client.is_authenticated().await
    }
...
#[async_trait]
impl ClaudeGateClient for MemoryStorageClient {
    async fn is_authenticated(&self) -> crate::claude_gate::Result<bool> {
        self.client.is_authenticated().await
    }
...
impl ClaudeGateProvider {
    ...
    pub fn with_storage(backend: StorageBackend, model: String, max_tokens: u32) -> Result<Self> {
        let (client, storage_name): (Arc<dyn ClaudeGateClient>, String) = match backend {
            StorageBackend::File => {
    ...
    pub async fn logout(&self) -> Result<()> {
        ...
        info!("Logged out from Claude Gate");
        Ok(())
    }
    ...
    pub async fn get_status(&self) -> ClaudeGateStatus {
...
impl LLMProvider for ClaudeGateProvider {
    ...
    async fn health_check(&self) -> bool {
        match self.client.is_authenticated().await {
            Ok(true) => true,
            Ok(false) => {
                debug!("Claude Gate health check: not authenticated");
                false
            }
            Err(e) => {
                warn!("Claude Gate health check failed: {}", e);
                false
            }
        }
    }
    ...
    async fn chat(&self, request: ChatRequest) -> Result<ChatResponse> {
        ...
        debug!(
            ...
            max_tokens = max_tokens,
            "Sending chat request via Claude Gate"
        );
        ...
        let response = self.client
            ...
            .map_err(|e| {
                if e.requires_reauth() {
                    ...
                    match &e {
                        crate::claude_gate::Error::Api { status, message, .. } => {
                            if *status == 429 {
        ...
        info!(
            ...
            input_tokens = response.usage.input_tokens,
            output_tokens = response.usage.output_tokens,
            "Received response from Claude Gate"
        );
    ...
    async fn stream_chat(
        ...
        debug!(
            ...
            message_count = messages.len(),
            max_tokens = max_tokens,
            "Starting streaming chat via Claude Gate"
        );
...
mod tests {
    ...
    #[test]
    fn test_provider_id() {
        let provider = ClaudeGateProvider::with_memory().unwrap();
        assert_eq!(provider.id(), "claude-gate");
    ...
    fn test_from_storage_name() {
        let provider = ClaudeGateProvider::from_storage_name(
            "memory",
    ...
    fn test_invalid_storage_name() {
        let result = ClaudeGateProvider::from_storage_name(
            "invalid",
    ...
    fn test_status_default() {
        let status = ClaudeGateStatus::default();
