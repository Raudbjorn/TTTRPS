...
//!
//! ```rust,ignore
//! use crate::gate::gemini::{CloudCodeClient, FileTokenStorage, MessagesRequest};
//!
//! # async fn example() -> crate::gate::gemini::Result<()> {
...
use super::auth::{discover_project, OAuthFlow};
use super::constants::{
    is_thinking_model, ANTIGRAVITY_SYSTEM_INSTRUCTION, API_PATH_GENERATE_CONTENT,
    API_PATH_STREAM_GENERATE_CONTENT,
};
use super::convert::{convert_request, convert_response, SignatureCache};
use super::error::{AuthError, Error, Result};
use super::models::content::ContentBlock;
use super::models::google::{CloudCodeWrapper, Content, GoogleRequest, Part};
use super::models::request::{Message, MessageContent, MessagesRequest, Role, SystemPrompt};
use super::models::response::{MessagesResponse, StopReason, Usage};
use super::models::stream::StreamEvent;
use super::models::tools::{Tool, ToolChoice};
use crate::gate::storage::TokenStorage;
use super::transport::http::{generate_request_id, HttpClient};
use super::transport::sse::SseStream;
...
impl<S: TokenStorage + 'static> CloudCodeClient<S> {
    ...
    /// ```
    pub async fn start_oauth_flow(&self) -> Result<(String, super::auth::OAuthFlowState)> {
    ...
    pub async fn request(&self, request: &MessagesRequest) -> Result<MessagesResponse> {
        ...
        // Parse response
        let google_response: super::models::google::GoogleResponse = response.json().await?;
        ...
        while let Some(event) = sse_stream.next().await {
            ...
            match event {
                ...
                StreamEvent::ContentBlockDelta { delta, .. } => {
                    match delta {
                        super::models::stream::ContentDelta::TextDelta { text } => {
                            current_text.push_str(&text);
                        }
                        super::models::stream::ContentDelta::ThinkingDelta { thinking } => {
                            current_thinking.push_str(&thinking);
                        }
                        super::models::stream::ContentDelta::SignatureDelta { signature } => {
                            current_signature = Some(signature);
                        }
                        super::models::stream::ContentDelta::InputJsonDelta { partial_json: _ } => {
...
///
/// ```rust,ignore
/// use crate::gate::gemini::{CloudCodeClient, FileTokenStorage};
...
pub struct CloudCodeClientBuilder<S: TokenStorage> {
    storage: Option<S>,
    http_builder: super::transport::http::HttpClientBuilder,
    oauth_config: Option<super::constants::OAuthConfig>,
...
impl<S: TokenStorage + 'static> CloudCodeClientBuilder<S> {
    ...
    pub fn with_oauth_config(mut self, config: super::constants::OAuthConfig) -> Self {
    ...
    pub fn build(self) -> CloudCodeClient<S> {
        ...
        let oauth = if let Some(config) = self.oauth_config {
            OAuthFlow::with_config(storage, config)
        } else {
            OAuthFlow::new(storage)
        };
...
impl<S: TokenStorage> Default for CloudCodeClientBuilder<S> {
    fn default() -> Self {
        Self {
            storage: None,
            http_builder: super::transport::http::HttpClientBuilder::default(),
            oauth_config: None,
...
impl<S: TokenStorage + 'static> MessagesRequestBuilder<S> {
    ...
    pub fn system_blocks(mut self, blocks: Vec<super::models::request::SystemBlock>) -> Self {
