...
///
/// ```rust,ignore
/// use crate::gate::gemini::convert::convert_request;
/// use crate::gate::gemini::MessagesRequest;
///
...
pub(crate) fn convert_request(request: &MessagesRequest) -> GoogleRequest {
        ...
        if !tools.is_empty() {
            ...
            if is_claude {
                google_request.tool_config = Some(ToolConfig {
                    function_calling_config: crate::gate::gemini::models::google::FunctionCallingConfig {
    ...
    if let Some(tool_choice) = &request.tool_choice {
        let new_config = convert_tool_choice(tool_choice);
        
        // Logic to preserve VALIDATED mode for Claude
        let should_overwrite = if let Some(current_config) = &mut google_request.tool_config {
            if current_config.function_calling_config.mode == "VALIDATED" {
                match new_config.function_calling_config.mode.as_str() {
                    "AUTO" => false, // Keep VALIDATED as superior AUTO
                    "ANY" => {
                        // Keep VALIDATED but apply constraints
                        current_config.function_calling_config.allowed_function_names = 
                            new_config.function_calling_config.allowed_function_names.clone();
                        false
                    },
                    "NONE" => true, // Explicitly disable tools
                    _ => true, // Should not happen given convert_tool_choice outputs
                }
            } else {
                true
            }
        ...
        if should_overwrite {
            google_request.tool_config = Some(new_config);
        }
    }
...
fn convert_system_prompt(system: &SystemPrompt) -> Vec<Part> {
    match system {
        ...
        SystemPrompt::Blocks(blocks) => blocks
            .iter()
            .filter_map(|block| {
                let crate::gate::gemini::models::request::SystemBlock::Text { text, .. } = block;
                if text.trim().is_empty() {
                    None
                } else {
                    Some(Part::text(text))
...
fn convert_tools(tools: &[crate::gate::gemini::models::tools::Tool]) -> Vec<FunctionDeclaration> {
    tools
        .iter()
        .enumerate()
        .map(|(idx, tool)| {
...
fn convert_tool_choice(choice: &crate::gate::gemini::models::tools::ToolChoice) -> ToolConfig {
    match choice {
        crate::gate::gemini::models::tools::ToolChoice::Auto => ToolConfig::auto(),
        crate::gate::gemini::models::tools::ToolChoice::Any => ToolConfig::any(),
        crate::gate::gemini::models::tools::ToolChoice::None => ToolConfig::none(),
        crate::gate::gemini::models::tools::ToolChoice::Tool { name } => ToolConfig::force(name),
    }
}
...
mod tests {
    use super::*;
    use crate::gate::gemini::models::tools::Tool;
    use serde_json::json;
    ...
    fn test_convert_tool_choice_auto() {
        let config = convert_tool_choice(&crate::gate::gemini::models::tools::ToolChoice::Auto);
        assert_eq!(config.function_calling_config.mode, "AUTO");
    }

    #[test]
    fn test_convert_tool_choice_any() {
        let config = convert_tool_choice(&crate::gate::gemini::models::tools::ToolChoice::Any);
        assert_eq!(config.function_calling_config.mode, "ANY");
    }

    #[test]
    fn test_convert_tool_choice_none() {
        let config = convert_tool_choice(&crate::gate::gemini::models::tools::ToolChoice::None);
        assert_eq!(config.function_calling_config.mode, "NONE");
    }

    #[test]
    fn test_convert_tool_choice_specific() {
        let config = convert_tool_choice(&crate::gate::gemini::models::tools::ToolChoice::tool("my_tool"));
        assert_eq!(config.function_calling_config.mode, "ANY");
        assert_eq!(
    ...
    fn test_convert_system_blocks() {
        use crate::gate::gemini::models::request::SystemBlock;
