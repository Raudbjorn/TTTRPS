...
use super::oauth::{self, OAuthFlowState};
use crate::gate::gemini::constants::{OAuthConfig, DEFAULT_OAUTH_CONFIG};
...
impl<S: TokenStorage> OAuthFlow<S> {
    ...
    pub async fn exchange_code(&self, code: &str, state: Option<&str>) -> Result<TokenInfo> {
        ...
        // Save the token
        self.storage.save("gemini", &token).await?;
    ...
    pub async fn is_authenticated(&self) -> Result<bool> {
        Ok(self.storage.exists("gemini").await?)
    }
    ...
    pub async fn logout(&self) -> Result<()> {
        ...
        // Remove stored token
        self.storage.remove("gemini").await?;
