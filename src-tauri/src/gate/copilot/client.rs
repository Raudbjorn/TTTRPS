...
use crate::gate::copilot::api::chat::ChatRequestBuilder;
use crate::gate::copilot::api::embeddings::EmbeddingsRequestBuilder;
use crate::gate::copilot::auth::constants::{API_VERSION, EDITOR_PLUGIN_VERSION};
use crate::gate::copilot::auth::token_exchange::TokenExchangeConfig;
use crate::gate::copilot::auth::{ensure_valid_copilot_token, DeviceFlowPending, PollResult};
use crate::gate::copilot::config::CopilotConfig;
use crate::gate::copilot::error::{Error, Result};
use crate::gate::copilot::models::{ModelsResponse, TokenInfo};
use crate::gate::copilot::storage::{CopilotTokenStorage, MemoryTokenStorage};
...
///
/// ```no_run
/// use crate::gate::copilot::CopilotClient;
///
/// # async fn example() -> crate::gate::copilot::Result<()> {
...
impl<S: CopilotTokenStorage> CopilotClient<S> {
    ...
    pub async fn start_device_flow(&self) -> Result<DeviceFlowPending> {
        crate::gate::copilot::auth::device_flow::start_device_flow(&self.http_client).await
    }
    ...
    pub async fn poll_for_token(&self, pending: &DeviceFlowPending) -> Result<PollResult> {
        crate::gate::copilot::auth::device_flow::poll_for_token(
            &self.http_client,
            &pending.device_code,
        )
        .await
    ...
    fn build_headers(&self, token: &TokenInfo) -> Result<HeaderMap> {
        ...
        headers.insert(
            USER_AGENT,
            HeaderValue::from_static(crate::gate::copilot::auth::constants::USER_AGENT),
        );
