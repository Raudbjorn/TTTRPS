...
///
/// Tokens are proactively refreshed this many seconds before expiry
/// to avoid failed API calls during the refresh window.
...
pub struct TokenInfo {
    ...
    ///
    /// Accepts "type" as the JSON key name for compatibility with existing formats.
    ...
    /// Accepts "access" as an alias for backward compatibility.
    #[serde(alias = "access")]
    ...
    /// Accepts "refresh" as an alias for backward compatibility.
    #[serde(alias = "refresh")]
    pub refresh_token: String,

    /// Unix timestamp when the access token expires.
    ///
    /// Accepts "expires" as an alias for backward compatibility.
    #[serde(alias = "expires")]
    pub expires_at: i64,

    /// Optional provider identifier (e.g., "anthropic", "gemini").
...
impl TokenInfo {
    ...
    /// indicating it should be refreshed before making API calls
    /// to avoid failures during the refresh window.
    ///
    /// # Example
    ///
    /// ```rust,ignore
    ...
    /// assert!(!token.needs_refresh()); // Fresh token
    ///
    /// let soon = TokenInfo::new("access".into(), "refresh".into(), 240); // 4 min
    /// assert!(soon.needs_refresh()); // Within 5-minute buffer
    /// ```
    #[must_use]
...
mod tests {
    ...
    fn test_with_provider() {
        ...
        assert_eq!(token.provider.as_deref(), Some("anthropic"));
    }
    ...
    fn test_serialization() {
        ...
        // Note: token_type is renamed to "type" in serde
    ...
    #[test]
    fn test_serialization_skips_none_provider() {
        let token = TokenInfo::new("access".into(), "refresh".into(), 3600);
        let json = serde_json::to_string(&token).unwrap();
        assert!(!json.contains("provider"));

        let token = token.with_provider("anthropic");
        let json = serde_json::to_string(&token).unwrap();
        assert!(json.contains("provider"));
    ...
    fn test_needs_refresh() {
        ...
        let soon = TokenInfo::with_expires_at(
            "access".into(),
            "refresh".into(),
            chrono::Utc::now().timestamp() + 240, // 4 minutes
        );
        ...
        let later = TokenInfo::with_expires_at(
            "access".into(),
            "refresh".into(),
            chrono::Utc::now().timestamp() + 360, // 6 minutes
        );
    ...
    fn test_backward_compat_aliases() {
        // Test that we can deserialize from the claude_gate format (short field names)
        let json = r#"{
            "type": "oauth",
    ...
    #[test]
    fn test_backward_compat_full_names() {
        // Test that we can deserialize with full field names
        let json = r#"{
            "type": "oauth",
    ...
    fn test_serialization_uses_full_names() {
        // Verify that serialization uses full field names (for forward compat)
        let token = TokenInfo::new("access".into(), "refresh".into(), 3600);
        let json = serde_json::to_string(&token).unwrap();

        // Should use "type", "access_token", "refresh_token", "expires_at"
        assert!(json.contains("\"type\""));
        assert!(json.contains("\"access_token\""));
        assert!(json.contains("\"refresh_token\""));
        assert!(json.contains("\"expires_at\""));

        // Should NOT use short names in output
