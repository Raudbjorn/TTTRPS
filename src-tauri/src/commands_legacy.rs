...
// Note: All Gate OAuth imports removed - extracted to commands/oauth/ modules
...
// Archetype Registry imports (types used by AppState struct)
...
// Archetype types are now imported by the archetype module directly
// See: commands/archetype/types.rs
...
// ============================================================================
// OAuth State Types - Extracted to commands/oauth module
// ============================================================================
// The OAuth state types (ClaudeGateState, GeminiGateState, CopilotGateState)
// and their storage backend enums have been extracted to:
// - src/commands/oauth/claude.rs
// - src/commands/oauth/gemini.rs
// - src/commands/oauth/copilot.rs
//
// Import them from the oauth module:
pub use crate::commands::oauth::{
...
/* EXTRACTED TO commands/oauth/claude.rs, commands/oauth/gemini.rs, commands/oauth/copilot.rs
// ============================================================================
...
// ============================================================================
// Copilot Gate State (OAuth Client for GitHub Copilot using Device Code Flow)
// ============================================================================
...
trait CopilotGateClientOps: Send + Sync {
    async fn is_authenticated(&self) -> Result<bool, String>;
    async fn get_token_info(&self) -> Result<Option<crate::gate::copilot::models::TokenInfo>, String>;
...
impl CopilotGateClientOps for CopilotFileStorageClientWrapper {
    ...
    async fn get_token_info(&self) -> Result<Option<crate::gate::copilot::models::TokenInfo>, String> {
        self.client.storage().load().await.map_err(|e| e.to_string())
    }

    async fn start_device_flow(&self) -> Result<DeviceFlowPending, String> {
        self.client.start_device_flow().await.map_err(|e| e.to_string())
    }

    async fn poll_for_token(&self, pending: &DeviceFlowPending) -> Result<CopilotPollResult, String> {
        self.client.poll_for_token(pending).await.map_err(|e| e.to_string())
    }

    async fn complete_auth(&self, github_token: String) -> Result<(), String> {
        self.client.complete_auth(github_token).await.map_err(|e| e.to_string())
    }
...
impl CopilotGateState {
    /// Create a client for the specified backend
    fn create_client(backend: CopilotGateStorageBackend) -> Result<Box<dyn CopilotGateClientOps>, String> {
        match backend {
            ...
            CopilotGateStorageBackend::Keyring => {
                ...
                log::warn!("Keyring storage for Copilot is not yet implemented, using file storage");
    ...
    pub async fn is_authenticated(&self) -> Result<bool, String> {
        let client = self.client.read().await;
        let client = client.as_ref().ok_or("Copilot Gate client not initialized")?;
        client.is_authenticated().await
    }

    /// Get token info
    pub async fn get_token_info(&self) -> Result<Option<crate::gate::copilot::models::TokenInfo>, String> {
        let client = self.client.read().await;
        let client = client.as_ref().ok_or("Copilot Gate client not initialized")?;
        client.get_token_info().await
    }
    ...
    pub async fn poll_for_token(&self, device_code: &str) -> Result<CopilotPollResult, String> {
        ...
        let pending = pending.as_ref().ok_or("No pending device flow. Call start_device_flow first.")?;
    ...
    /// Switch to a different storage backend
    pub async fn switch_backend(&self, new_backend: CopilotGateStorageBackend) -> Result<String, String> {
        let new_client = Self::create_client(new_backend)?;
        let backend_name = new_client.storage_name();
        ...
        log::info!("Switched Copilot Gate storage backend to: {}", backend_name);
        Ok(backend_name.to_string())
    }
}
END OF EXTRACTED OAUTH STATE TYPES */
...
pub struct AppState {
    ...
    // Copilot Gate OAuth client (GitHub Copilot using Device Code Flow)
...
impl AppState {
    pub fn init_defaults() -> (
        ...
        // Initialize Copilot Gate client (Device Code Flow for GitHub Copilot)
...
// EXTRACTED TO: src/commands/archetype/
// See: commands/archetype/crud.rs, vocabulary.rs, setting_packs.rs, resolution.rs
// ============================================================================

/*
// ============================================================================
// Request/Response Types for Archetype Commands - EXTRACTED
// ============================================================================
...
*/
// End of extracted archetype section
