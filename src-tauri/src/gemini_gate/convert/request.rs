        ...
        if !tools.is_empty() {
            ...
            if is_claude {
                google_request.tool_config = Some(ToolConfig {
                    function_calling_config: crate::gemini_gate::models::google::FunctionCallingConfig {
...
fn convert_system_prompt(system: &SystemPrompt) -> Vec<Part> {
    match system {
        ...
        SystemPrompt::Blocks(blocks) => blocks
            .iter()
            .filter_map(|block| {
                let crate::gemini_gate::models::request::SystemBlock::Text { text, .. } = block;
                if text.is_empty() {
                    None
                } else {
                    Some(Part::text(text))
...
fn convert_tools(tools: &[crate::gemini_gate::models::tools::Tool]) -> Vec<FunctionDeclaration> {
    tools
        .iter()
        .enumerate()
        .map(|(idx, tool)| {
...
fn convert_tool_choice(choice: &crate::gemini_gate::models::tools::ToolChoice) -> ToolConfig {
    match choice {
        crate::gemini_gate::models::tools::ToolChoice::Auto => ToolConfig::auto(),
        crate::gemini_gate::models::tools::ToolChoice::Any => ToolConfig::any(),
        crate::gemini_gate::models::tools::ToolChoice::None => ToolConfig::none(),
        crate::gemini_gate::models::tools::ToolChoice::Tool { name } => ToolConfig::force(name),
    }
}
...
mod tests {
    use super::*;
    use crate::gemini_gate::models::tools::Tool;
    use serde_json::json;
    ...
    fn test_convert_tool_choice_auto() {
        let config = convert_tool_choice(&crate::gemini_gate::models::tools::ToolChoice::Auto);
        assert_eq!(config.function_calling_config.mode, "AUTO");
    }

    #[test]
    fn test_convert_tool_choice_any() {
        let config = convert_tool_choice(&crate::gemini_gate::models::tools::ToolChoice::Any);
        assert_eq!(config.function_calling_config.mode, "ANY");
    }

    #[test]
    fn test_convert_tool_choice_none() {
        let config = convert_tool_choice(&crate::gemini_gate::models::tools::ToolChoice::None);
        assert_eq!(config.function_calling_config.mode, "NONE");
    }

    #[test]
    fn test_convert_tool_choice_specific() {
        let config = convert_tool_choice(&crate::gemini_gate::models::tools::ToolChoice::tool("my_tool"));
        assert_eq!(config.function_calling_config.mode, "ANY");
        assert_eq!(
    ...
    fn test_convert_system_blocks() {
        use crate::gemini_gate::models::request::SystemBlock;
