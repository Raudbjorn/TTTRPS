//! Reusable Claude Gate OAuth authentication component.
//!
//! This component provides a complete OAuth authentication flow UI for Claude Gate,
//! including status display, storage backend selection, login/logout buttons, and
//! auth code input. It can be used in any settings panel that needs Claude Gate auth.
...
pub fn ClaudeGateAuth(
    ...
    #[prop(optional)]
    on_status_change: Option<Callback<ClaudeGateStatus>>,
    ...
    // Internal state
    let status = RwSignal::new(ClaudeGateStatus::default());
    ...
    let change_storage = move |backend: ClaudeGateStorageBackend| {
        spawn_local(async move {
            is_loading.set(true);
...
/// Compact status indicator for Claude Gate authentication.
...
pub fn ClaudeGateStatusBadge() -> impl IntoView {
    ...
    Effect::new(move |_| {
        spawn_local(async move {
            if let Ok(s) = claude_gate_get_status().await {
                status.set(s);
            }
        });
    });
